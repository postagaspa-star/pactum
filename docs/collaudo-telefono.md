...
Trova l'IP del PC nella rete: `ipconfig` → "Indirizzo IPv4" (es. `192.168.1.50`). L'indirizzo del server per i telefoni sarà `http://192.168.1.50:8000`.
...
Apri sul telefono: `http://192.168.1.50:8000/scarica` (con gli APK di debug servi tu i file, oppure usa i link diretti). La pagina spiega quale app installa chi. In alternativa copia gli APK via cavo.
...
4. **Impostazioni** dell'app → indirizzo server `http://192.168.1.50:8000` + il token del figlio (di default `dev-token-figlio`; in produzione lo cambierai). Premi **"Prova adesso"** → deve dire "battito consegnato".
...
- **Chiave di firma**: `C:\Users\andre\pactum-keys` va copiata in un posto sicuro — persa quella, niente più aggiornamenti.
