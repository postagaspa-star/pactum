...
- `dettagli` per tipo:
  - **uso_giornaliero** — fotografia cumulativa del giorno (il server tiene l'ultima ricevuta per `giorno`): `{ "giorno": "YYYY-MM-DD", "uso_minuti": {package: minuti}, "totale_minuti": n }`. Il `giorno` è il giorno locale del telefono.
...
- PATCH: corpo `{ "parametri": {...}, "proposta_id": n? }` — `proposta_id` (proposta accettata, monouso) bypassa il lock dei 4 giorni (modifica concordata).
...
- DELETE = allentamento massimo (stesso lock) e soft-delete; eliminare l'ultima regola attiva → `409 errore=ultima_regola`.
