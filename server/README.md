...
4. Token dev: figlio `dev-token-figlio`, genitore `dev-token-genitore` — in produzione impostare `PACTUM_TOKEN_FIGLIO`, `PACTUM_TOKEN_GENITORE` e `PACTUM_DB` (percorso del database). Opzionale: `PACTUM_TIMEZONE` (fuso del patto per i bucket giorno/settimana, default `Europe/Rome`)
   ...
   - (v3.1) Prima di migrare il server fa una copia completa accanto al database: `<db>.prima-v3-AAAAMMGG-HHMMSS` (ora del patto, di norma Roma; in Docker sta nel volume, accanto a `/data/pactum.db`). Se la copia non riesce il server **non parte** e il log dice perché (riga `MIGRAZIONE v3 FERMATA`). Su un database nuovo o già v3 la copia non si fa.
