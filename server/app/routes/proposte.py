...
def formatta_proposta(riga: sqlite3.Row) -> dict:
    return {
        "id": riga["id"],
        "regola_id": riga["regola_id"],
        "parametri_proposti": json.loads(riga["parametri_proposti"])
        if riga["parametri_proposti"]
        else None,
        "motivazione": riga["motivazione"],
        "confronto": riga["confronto"],
...
def crea_proposta(
    ...
    gia_pendente = conn.execute(
        "SELECT 1 FROM proposte WHERE regola_id = ? AND stato = 'pendente'", (corpo.regola_id,)
    ).fetchone()
    ...
    conn.commit()
    return formatta_proposta(_proposta_o_404(conn, proposta_id))
...
def elenca_proposte(
    ...
    return {"proposte": [formatta_proposta(r) for r in righe]}
...
def rispondi_proposta(
    ...
    ora = clock.now()
    ts = clock.iso(ora)
    parametri = json.loads(proposta["parametri_proposti"]) if proposta["parametri_proposti"] else None
    ...
    conn.execute(
        "UPDATE proposte SET stato = ?, usata = ?,"
        " risposta_esito = ?, risposta_motivazione = ?, risposta_ts = ? WHERE id = ?",
        (stato, usata, corpo.esito, corpo.motivazione, ts, proposta_id),
    )
    accoda_notifica(
        conn,
        "proposta_risposta",
    ...
    conn.commit()
    return {"proposta": formatta_proposta(_proposta_o_404(conn, proposta_id)), "regola": regola_risultante}
